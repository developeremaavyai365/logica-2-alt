  ...
  return (
    <div className="w-full bg-[#dbe8d6]">
        ...
        <div className="max-w-4xl mx-auto flex flex-wrap justify-center gap-2 px-4 sm:px-6 md:px-10 pb-10">
          {aboutLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="text-xs font-medium px-4 py-2 rounded-full bg-white border border-[#1f2a1d]/10 text-[#4b5b47] hover:text-[#1f2a1d] hover:border-[#1f2a1d]/30 transition-colors"
            >
              {link.label}
            </Link>
          ))}
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 md:px-10 py-16 sm:py-20">
        <p className="text-[#4b5b47] text-sm sm:text-base leading-relaxed">
        ...
        <div className="mt-10 grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6">
          {scaleStats.map((stat) => (
            <div key={stat.label} className="rounded-2xl bg-[#f4f8f3] border border-[#1f2a1d]/10 p-5 text-center">
              <div className="text-2xl font-semibold text-[#1f2a1d]">{stat.value}</div>
              <div className="text-xs text-[#4b5b47] mt-1">{stat.label}</div>
            </div>
          ))}
        </div>
        <p className="mt-3 text-xs text-[#4b5b47]/70">As of March 31, 2025.</p>

        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div className="rounded-2xl border border-[#1f2a1d]/10 p-5">
            <h3 className="text-sm font-semibold text-[#1f2a1d] mb-2">Financial performance</h3>
          ...
          <div className="rounded-2xl border border-[#1f2a1d]/10 p-5">
            <h3 className="text-sm font-semibold text-[#1f2a1d] mb-2">Registered office</h3>
            <p className="text-sm text-[#4b5b47] leading-relaxed">{REGISTERED_OFFICE.address}</p>
            <p className="mt-1 text-xs text-[#4b5b47]/70">{REGISTERED_OFFICE.relocatedNote}</p>
          </div>
        </div>
      </section>
      ...
      <section className="w-full bg-[#f4f8f3] py-16 sm:py-20 px-4 sm:px-6 md:px-10">
        <div className="max-w-5xl mx-auto">
          <span className="text-[#3d5638] text-xs sm:text-sm font-semibold uppercase tracking-wide">Incorporation history</span>
          <h2 className="mt-3 text-2xl sm:text-3xl font-normal text-[#336443]" style={{ letterSpacing: '-0.03em' }}>
            ...
            {nameHistory.map((entry, i) => (
              <div
                ...
                className={`flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-6 py-4 ${
                  i !== nameHistory.length - 1 ? 'border-b border-[#1f2a1d]/10' : ''
                }`}
              >
                <span className="text-xs font-medium text-[#3d5638] sm:w-32 shrink-0">{entry.date}</span>
                <span className="text-sm font-semibold text-[#1f2a1d] sm:w-72 shrink-0">{entry.name}</span>
                <span className="text-sm text-[#4b5b47]">{entry.event}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 md:px-10 py-16 sm:py-20">
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-5">
          {pillars.map((pillar) => (
            <div key={pillar.title} className="rounded-2xl bg-[#f4f8f3] border border-[#1f2a1d]/10 p-6">
              <h3 className="text-base font-semibold text-[#1f2a1d] leading-snug">{pillar.title}</h3>
              <p className="mt-3 text-sm text-[#4b5b47] leading-relaxed">{pillar.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* Offices */}
      <section className="w-full bg-[#1f2a1d] py-16 sm:py-20 px-4 sm:px-6 md:px-10">
        <div className="max-w-5xl mx-auto">
