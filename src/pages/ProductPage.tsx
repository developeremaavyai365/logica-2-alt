  ...
  return (
    <div className="w-full bg-[#dbe8d6]">
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 md:px-10 py-10">
        <div className="flex items-center gap-1 text-xs text-[#4b5b47] mb-8">
          <Link to="/shop" className="hover:text-[#1f2a1d] transition-colors">Shop</Link>
          <span>/</span>
          {category && (
            <>
              <Link to={`/shop/${category.slug}`} className="hover:text-[#1f2a1d] transition-colors">{category.name}</Link>
              <span>/</span>
            </>
          )}
          <span className="text-[#1f2a1d]">{product.name}</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          {/* Info */}
          <div>
            <span className="text-xs font-medium text-[#3d5638] uppercase tracking-wide">{product.brand}</span>
            <h1 className="mt-2 text-2xl sm:text-3xl font-normal text-[#1f2a1d] leading-snug" style={{ letterSpacing: '-0.02em' }}>
              {product.name}
            </h1>

            <div className="mt-5 flex items-center gap-3 flex-wrap">
              <span className="text-2xl font-semibold text-[#1f2a1d]">{formatINR(product.price)}</span>
              {product.mrp && product.mrp > product.price && (
                <span className="text-sm text-[#4b5b47]/60 line-through">{formatINR(product.mrp)}</span>
              )}
              {discount > 0 && (
                <span className="text-xs font-semibold text-white bg-[#3d5638] px-2 py-1 rounded-full">{discount}% off</span>
              )}
            </div>

            <p className={`mt-3 text-sm font-medium ${product.inStock ? 'text-[#3d5638]' : 'text-red-600'}`}>
            ...
            <div className="mt-6 flex items-center gap-3 flex-wrap">
              ...
              <button
                ...
                className={`w-11 h-11 rounded-full border flex items-center justify-center transition-colors ${
                  inWishlist(product.id) ? 'bg-[#1f2a1d] border-[#1f2a1d] text-white' : 'border-[#1f2a1d]/20 text-[#1f2a1d] hover:border-[#1f2a1d]/50'
                }`}
              >
                <Heart className="w-4 h-4" fill={inWishlist(product.id) ? 'currentColor' : 'none'} />
              </button>
            </div>
            <Link to="/contact" className="mt-4 inline-block text-[#1f2a1d] text-sm font-semibold hover:opacity-80 transition-opacity">
              Enquire for bulk pricing →
            </Link>

            {detail?.highlights && detail.highlights.length > 0 && (
              <div className="mt-10">
                <h2 className="text-sm font-semibold text-[#1f2a1d] mb-3">Highlights</h2>
                <ul className="space-y-2">
                  {detail.highlights.map((h) => (
                    <li key={h} className="flex items-start gap-2 text-sm text-[#4b5b47] leading-relaxed">
                      <Check className="w-4 h-4 text-[#3d5638] shrink-0 mt-0.5" />
                      {h}
                    </li>
                  ))}
                </ul>
            ...
            {specRows.length > 0 && (
              <div className="mt-10">
                <h2 className="text-sm font-semibold text-[#1f2a1d] mb-3">Technical Details</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {specRows.map(([label, value]) => {
                    const Icon = iconForSpec(label);
                    return (
                      <div key={label} className="flex items-start gap-3 rounded-xl bg-[#f4f8f3] p-3.5">
                        <span className="w-8 h-8 rounded-lg bg-white flex items-center justify-center shrink-0">
                          <Icon className="w-4 h-4 text-[#3d5638]" />
                        </span>
                        <div className="min-w-0">
                          <p className="text-[10px] font-semibold uppercase tracking-wide text-[#4b5b47]/70">{label}</p>
                          <p className="text-sm font-medium text-[#1f2a1d] leading-snug break-words">{value}</p>
                        </div>
                      </div>
                    );
                  })}
            ...
            {detail?.overview && (
              <div className="mt-10">
                <h2 className="text-sm font-semibold text-[#1f2a1d] mb-3">Overview</h2>
                {(() => {
                  ...
                  return items ? (
                    ...
                    <p className="text-sm text-[#4b5b47] leading-relaxed">{detail.overview}</p>
                  );
                })()}
              </div>
            )}
        ...
        {related.length > 0 && (
          <div className="mt-20">
            <h2 className="text-xl font-semibold text-[#1f2a1d] mb-6">You may also like</h2>
